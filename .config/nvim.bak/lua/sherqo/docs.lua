...
local Groups = require("sherqo.groups")
...
function M.extras()
  local Extra = require("sherqo.extra")
...
function M.plugins()
  ...
  for _, name in ipairs(names) do
    ...
    t[#t + 1] = {
      link(repo, group.url),
      link(("`%s`"):format(name), "lua/sherqo/groups/" .. name .. ".lua"),
    }
  end
  return M.table(t)
end
...
function M.update()
  local config = Docs.extract("lua/sherqo/config.lua", "\n(--@class sherqo%.Config.-\n})")
